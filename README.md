# 안녕하세요, 지속적으로 성장하고 있는 신입 백엔드 개발자입니다.

## 📌 소개 및 기술적 지향점
* 웹 표준과 접근성을 준수하며, Zustand를 활용한 효율적인 상태 관리에 깊은 관심을 두고 있습니다.
* 백엔드에서 처리된 데이터가 화면에 유기적으로 연결되어, 사용자가 막힘없이 서비스를 이용하는 환경을 만드는 것에 흥미를 느낍니다.

---

## 🛠 기술 스택

### 프론트엔드
<img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white"/> <img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white"/> <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black"/> <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white"/> <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black"/> <img src="https://img.shields.io/badge/Zustand-443E38?style=flat-square&logo=react&logoColor=white"/>

### 백엔드
<img src="https://img.shields.io/badge/Java-007396?style=flat-square&logo=openjdk&logoColor=white"/> <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white"/> <img src="https://img.shields.io/badge/JPA_/_Hibernate-59666C?style=flat-square&logo=hibernate&logoColor=white"/> <img src="https://img.shields.io/badge/JWT-000000?style=flat-square&logo=jsonwebtokens&logoColor=white"/> <img src="https://img.shields.io/badge/OAuth2-323232?style=flat-square&logo=github&logoColor=white"/>

### 데이터베이스 및 도구
<img src="https://img.shields.io/badge/MariaDB-003545?style=flat-square&logo=mariadb&logoColor=white"/> <img src="https://img.shields.io/badge/DBeaver-382923?style=flat-square&logo=dbeaver&logoColor=white"/> <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white"/> <img src="https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=postman&logoColor=white"/> <img src="https://img.shields.io/badge/IntelliJ_IDEA-000000?style=flat-square&logo=intellijidea&logoColor=white"/> <img src="https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white"/>

---

## 💻 협업 태도 및 개발 성향
* **소통과 협업:** 독단적인 개발보다 팀원과의 원활한 소통을 최우선으로 생각합니다.
* **꼼꼼한 분석:** 코드의 동작 원리를 명확히 이해하고 넘어가는 꼼꼼한 성향을 지니고 있습니다.
* **유연한 태도:** 상황과 조건에 맞춰 유연하게 수정하고 타협점을 찾아낼 수 있습니다.
